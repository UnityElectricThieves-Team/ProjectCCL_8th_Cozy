...
- 마우스/키 입력 자체는 다루지 않는다 — 마우스 라우팅은 [Interaction/CLAUDE.md](../Interaction/CLAUDE.md)의 인터페이스(`IHoverable` 등)를 통해, OS-wide 키 입력은 [Platform/Input/GlobalKeyInput](../Platform/Input/GlobalKeyInput.cs)을 *구독*해서 받는다.
...
- `CharacterBasicAI2D.cs` — Idle/Walk/Sleep/WakeUp/Fall/Land 6-상태 자율 거동. 친밀도와 분리된 단일 책임 컴포넌트.
  ...
  - `RequestSleep` / `RequestWakeUp` / `RequestFall` public 메서드로 외부 트리거 수신 — Sleep은 같은 폴더의 [SleepController](SleepController.cs)가 호출. Sleep/Fall/Land 중에는 RequestSleep 무시 (짧은 트랜지션 상태에선 외부 트리거 보류).
...
- `SleepController.cs` — 씬 전역 수면 정책(*씬-레벨 캐릭터 조정자*). 임계 시간 무입력이면 씬의 모든 `CharacterBasicAI2D`에 `RequestSleep` 일괄 호출, 입력 감지 시 `RequestWakeUp`. 입력 소스는 [Platform/Input/GlobalKeyInput](../Platform/Input/GlobalKeyInput.cs) (OS-wide 키) + `Mouse.current` (창 포커스 한정).
...
- **OS-wide 키 입력은 [Platform/Input/GlobalKeyInput](../Platform/Input/GlobalKeyInput.cs) 구독으로**. GlobalKeyInput 자체가 두 OS 경로(WH_KEYBOARD_LL + InputSystem)를 단일 이벤트로 추상화한 컴포넌트라, Character가 *추상화된 결과만 소비*하는 형태. OS 호출(Win32 P/Invoke 등)은 직접 하지 않는다.
